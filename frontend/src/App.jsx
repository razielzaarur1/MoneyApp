...
// --- Category Tree (Based on the provided images) ---
const INCOMES = {
...
// Helper to find category details by subCategoryId
const getCategoryDetails = (subCatId) => {
  if (!subCatId) return { mainCat: EXPENSES[10], subCat: EXPENSES[10].subs[4] }; // Default: שונות -> ללא סיווג
...
export default function App() {
  ...
  // זכרון קטגוריות חכם (Machine Learning סימולציה)
  ...
  const fetchData = () => {
    setLoading(true);
    // סימולציה של שרת (תחליף עם ה-fetch האמיתי שלך)
    setTimeout(() => {
      const mockTransactions = [
        { id: '1', date: '28/02/2026', description: 'וטירינר תורן', amount: -929.00, account: 'max (9324)', categoryId: 'fam_pets' },
        { id: '2', date: '28/02/2026', description: 'חי בעיר', amount: -450.00, account: 'max (9324)', categoryId: 'fam_pets' },
        { id: '3', date: '02/03/2026', description: 'מכינת חמדת יהודה', amount: 0.00, account: 'max (9324)', categoryId: 'fam_school' },
        { id: '4', date: '22/02/2026', description: 'סעוד הראל - כללית', amount: -13.94, account: 'max (2623)', categoryId: 'hlth_services' },
        { id: '5', date: '22/02/2026', description: 'מקס כאן קונים בכיף', amount: -55.90, account: 'max (2623)', categoryId: 'shop_supermarket' },
        { id: '6', date: '22/02/2026', description: 'שרותי בריאות כללית הו"ק', amount: -80.87, account: 'max (2623)', categoryId: 'hlth_insurance' },
        { id: '7', date: '10/02/2026', description: 'משכורת חודשית', amount: 12500.00, account: 'בנק לאומי', categoryId: 'inc_salary' },
      ];
      
      const mappedTransactions = mockTransactions.map(tx => ({
        ...tx,
        // שימוש בזיכרון בתי העסק אם קיים
        categoryId: merchantMapping[tx.description] || tx.categoryId || 'misc_uncategorized'
      }));
      
      setTransactions(mappedTransactions);
      setAccounts([
        { id: '1', name: 'בנק לאומי', type: 'bank', balance: 14500, status: 'success' },
        { id: '2', name: 'max (9324)', type: 'credit', balance: -1379, status: 'success' },
        { id: '3', name: 'max (2623)', type: 'credit', balance: -150.71, status: 'success' }
      ]);
  ...
  // Handler: Update category + Learn for future!
  const handleUpdateTxCategory = (txId, newSubCatId) => {
    const txToUpdate = transactions.find(t => t.id === txId);
    if(txToUpdate) {
      const desc = txToUpdate.description;
      // 1. נשמור בזיכרון: כל תנועה עתידית או קיימת עם התיאור הזה תקבל את הקטגוריה החדשה
      setMerchantMapping(prev => ({ ...prev, [desc]: newSubCatId }));
      
      // 2. נעדכן את כל התנועות בהיסטוריה שיש להן את אותו תיאור בדיוק
      setTransactions(transactions.map(tx => 
        tx.description === desc ? { ...tx, categoryId: newSubCatId } : tx
      ));
    }
    setSelectedTx(null);
  };
...
function OverviewView({ filteredTransactions, accounts, onTxClick, getCategoryDetails, getAccountName }) {
              ...
              <table className="w-full text-right">
                <thead className="bg-gray-50/80 text-gray-500 text-sm border-b border-gray-100">
                  <tr>
                    ...
                    <th className="px-4 py-3 font-medium">תיאור וקטגוריה</th>
                    <th className="px-4 py-3 font-medium">חשבון מחיוב</th>
                  ...
                  {filteredTransactions.slice(0, 5).map((tx) => {
                    ...
                    return (
                      <tr key={tx.id} onClick={() => onTxClick(tx)} className="hover:bg-blue-50/30 transition-colors cursor-pointer group">
                        <td className="px-4 py-3 text-xs text-gray-500 w-24">{tx.date}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className={`p-1.5 rounded-lg ${mainCat.bg} ${mainCat.color} shrink-0`}>
                              <Icon size={16} />
                            </div>
                            <div className="flex flex-col">
                              <span className="text-sm font-bold text-gray-800 leading-tight">{tx.description}</span>
                              <span className="text-[11px] text-gray-500 mt-0.5">{mainCat.name} • {subCat.name}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-xs text-gray-500">{getAccountName(tx.accountId || tx.account)}</td>
                        <td className={`px-4 py-3 text-sm font-bold ${tx.amount > 0 ? 'text-green-600' : 'text-gray-800'} text-left`} dir="ltr">
...
function TransactionsView({ filteredTransactions, ALL_CATEGORIES, onTxClick, getCategoryDetails, getAccountName }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-in fade-in duration-500">
      <div className="p-4 sm:p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-gray-50/30">
        <div className="flex gap-2 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0 hide-scrollbar">
          <button onClick={() => setFilterType('all')} className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${filterType === 'all' ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
            כל התנועות
          </button>
          <button onClick={() => setFilterType('expense')} className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${filterType === 'expense' ? 'bg-red-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
            הוצאות בלבד
          </button>
          <button onClick={() => setFilterType('income')} className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${filterType === 'income' ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
            הכנסות בלבד
          </button>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-right">
          <thead className="bg-white text-gray-400 text-xs uppercase tracking-wider">
            <tr>
              <th className="px-4 py-3 font-semibold border-b border-gray-100">תאריך</th>
              <th className="px-4 py-3 font-semibold border-b border-gray-100">תיאור וקטגוריה</th>
              <th className="px-4 py-3 font-semibold border-b border-gray-100 hidden sm:table-cell">חשבון מחיוב</th>
              <th className="px-4 py-3 font-semibold border-b border-gray-100">סכום</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {displayedTxs.map((tx) => {
              ...
              const Icon = IconMap[mainCat.icon] || Tag;
              return (
                <tr key={tx.id} onClick={() => onTxClick(tx)} className="hover:bg-blue-50 transition-colors cursor-pointer group">
                  <td className="px-4 py-2.5 text-xs text-gray-500 w-20 sm:w-28">{tx.date}</td>
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${mainCat.bg} ${mainCat.color} shrink-0`}>
                        <Icon size={16} />
                      </div>
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-gray-800 leading-tight">{tx.description}</span>
                  ...
                  <td className={`px-4 py-2.5 text-sm font-bold ${tx.amount > 0 ? 'text-green-600' : 'text-gray-900'} text-left whitespace-nowrap`} dir="ltr">
                    {tx.amount > 0 ? '+' : ''}{tx.amount.toFixed(2)} ₪
                  </td>
                </tr>
              );
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-100 bg-gray-50/50">
          <h2 className="text-lg font-bold text-gray-800">הגדרות אבטחה ופרטיות</h2>
        </div>
        <div className="divide-y divide-gray-100">
          <SettingsRow icon={<Shield size={22} />} label="הצפנה מתקדמת" description="ניהול מפתחות הצפנה עבור המידע המקומי" />
...
function TransactionModal({ tx, ALL_CATEGORIES, getCategoryDetails, accounts, onClose, onSave }) {
  ...
  const [selectedMainCat, setSelectedMainCat] = useState(currentDetails.mainCat.id);
  const [selectedSubCat, setSelectedSubCat] = useState(currentDetails.subCat.id);

  const acc = accounts.find(a => a.id === tx.accountId || a.id === tx.account);
  
  const currentMainCatObj = ALL_CATEGORIES.find(c => c.id === selectedMainCat) || ALL_CATEGORIES[0];

  const handleSave = () => {
    onSave(tx.id, selectedSubCat);
  };
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in">
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="bg-gray-50/80 p-6 border-b border-gray-100 flex justify-between items-start shrink-0">
           <div>
        ...
        <div className="p-4 border-b border-gray-100 text-center shrink-0">
          <p className={`text-3xl font-black ${tx.amount < 0 ? 'text-red-600' : 'text-green-600'}`} dir="ltr">
        ...
        <div className="p-6 bg-white overflow-y-auto flex-1 space-y-6">
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">1. בחר קטגוריה ראשית</label>
            <select 
              value={selectedMainCat} 
              onChange={(e) => {
                setSelectedMainCat(e.target.value);
                // Reset subcategory to first of new main category
                const newMainObj = ALL_CATEGORIES.find(c => c.id === e.target.value);
                if(newMainObj && newMainObj.subs.length > 0) {
                  setSelectedSubCat(newMainObj.subs[0].id);
                }
              }}
              className="w-full p-3 border border-gray-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 text-base shadow-sm font-medium"
            >
              <optgroup label="הוצאות">
                {EXPENSES.map(cat => <option key={cat.id} value={cat.id}>{cat.name}</option>)}
              </optgroup>
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">2. בחר תת-קטגוריה</label>
            <div className="grid grid-cols-1 gap-2">
              {currentMainCatObj.subs.map(sub => (
                <button
                  key={sub.id}
                  onClick={() => setSelectedSubCat(sub.id)}
                  className={`flex items-center justify-between p-3 rounded-xl border transition-all text-right
                    ${selectedSubCat === sub.id ? `border-blue-500 bg-blue-50 text-blue-800 font-bold ring-1 ring-blue-500` : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                >
                  <span className="text-sm">{sub.name}</span>
                  {selectedSubCat === sub.id && <Check size={16} className="text-blue-500" />}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-100 shrink-0">
          <button onClick={handleSave} className="w-full bg-gray-900 text-white font-bold py-3.5 rounded-xl hover:bg-black transition-colors shadow-md">
            שמור ולמד קטגוריה לעסק
          </button>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden relative animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <p className="text-sm text-gray-600 mb-4 bg-blue-50 p-3 rounded-lg border border-blue-100">
            הפרטים מוצפנים ונשמרים אך ורק במכשיר שלך.
          </p>
          ...
          <div>
            ...
            <input type="text" required className="w-full p-3 border border-gray-200 rounded-xl outline-none" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">סיסמה</label>
            <input type="password" required className="w-full p-3 border border-gray-200 rounded-xl outline-none text-left" dir="ltr" />
          </div>
          ...
          >
            {loading ? <RefreshCw size={20} className="animate-spin mx-auto" /> : 'התחבר וסנכרן נתונים'}
          </button>
        </form>
      </div>
    </div>
